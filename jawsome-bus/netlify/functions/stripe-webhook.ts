...
import { createPrintfulOrder } from "../../src/server/lib/printful";
  ...
  try {
    ...
    if (session.metadata?.type === "event_booking") {
      ...
      await sendEventBookingEmail({
        ...
        eventTitle:
          session.metadata.event_title || "Jawsome Derby Fun Bus Event",
        seats,
    ...
    if (session.metadata?.type === "store_order") {
      ...
      try {
        const printfulOrder = await createPrintfulOrder({
          orderNumber,
          recipient: {
            name: `${session.metadata?.shipping_first_name || ""} ${
              session.metadata?.shipping_last_name || ""
            }`.trim(),
            email: customerEmail,
            ...
            country_code:
              session.metadata?.shipping_country_code || "US",
          },
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        message: "No booking update needed",
      }),
    };
  } catch (error) {
